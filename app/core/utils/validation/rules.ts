...
export const getValidationRules = (t: TranslateFn) => {
  const required: ValidationRule = (v) => {
    if (v === null || v === undefined) return t('required')
    const value = String(v)
    return (value && value.trim().length > 0) || t('required')
  }

  const requiredWithoutTrim: ValidationRule = (v) => {
    if (v === null || v === undefined) return t('requiredWithoutTrim')
  ...
  const minNumber = (min: number): ValidationRule => (v) => {
    ...
    const num = Number(v)
    if (Number.isNaN(num)) return t('minNumber', { min })
  ...
  const maxNumber = (max: number): ValidationRule => (v) => {
    if (v === null || v === undefined || v === '') return true
    const num = Number(v)
    if (Number.isNaN(num)) return t('maxNumber', { max })
    return num <= max || t('maxNumber', { max })
  }

  const minMaxNumber = (min: number, max: number, required = true): ValidationRule => (v) => {
    if (!required && (v === null || v === undefined || v === '')) return true
    const num = Number(v)
    if (Number.isNaN(num)) return t('minMaxNumber', { min, max })
  ...
  return {
    phoneNumber, email, required, requiredWithoutTrim, maxLength, minLength,
    minMaxNumber, onlyUniqSymbols, agree, minNumber, maxNumber, twoDecimals, noDecimals,
  }
}
